...
namespace Hotel_Management_System.User_Control
{
    ...
    public partial class UserControlSettings : UserControl
    {
        ...
        private void Clear1()
        {
            textBoxUDUserName.Clear();
            textBoxUDPassword.Clear();
            ID = "";
        }
        ...
        private void buttonUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (ID != "")
            {
                ...
                {
                    check = db.UpdateUser(ID, textBoxUDUserName.Text.Trim(), textBoxUDPassword.Text.Trim());
                    if (check)
                        Clear1();
                }
            }
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            ...
            if (ID != "")
            {
                if (textBoxUDUserName.Text.Trim() == string.Empty || textBoxUDPassword.Text.Trim() == string.Empty)
                    MessageBox.Show("Please fill out all the fields.", "Required", MessageBoxButtons.OK, MessageBoxIcon.Information);
                else
                {
                    DialogResult result = MessageBox.Show("Do you want to delete this user?", "Delete Client", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void dataGridViewUser_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if(e.RowIndex != -1)
            {
                ...
                textBoxUDUserName.Text = row.Cells[1].Value.ToString();
                textBoxUDPassword.Text = row.Cells[2].Value.ToString();
